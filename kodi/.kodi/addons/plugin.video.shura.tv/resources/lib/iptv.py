...
CHFILE = os.path.join(xbmc.translatePath('special://profile/'), 'channels.txt')
...
class shura:
    def __init__(self, OTT, StreamType, ServerName, addonid=''):
        ...
        self.supported_settings = {
            'media_server_id': {'name': 'media_server_id', 'language_key': 34001, 'lookup_in': 'media_servers',
                                'lookup': 'id', 'display': 'title'},
            'time_shift': {'name': 'time_shift', 'language_key': 34002,
                           'defined': [('0', '0'), ('60', '1'), ('120', '2'), ('180', '3'), ('240', '4'), ('300', '5'),
                                       ('360', '6'), ('420', '7'), ('480', '8'), ('540', '9'), ('600', '10'),
                                       ('660', '11'), ('720', '12')]},
            'time_zone': {'name': 'time_zone', 'language_key': 34003,
                          'defined': [('-720', '-12 GMT (New Zealand Standard Time)'),
                                      ('-660', '-11 GMT (Midway Islands Time)'),
                                      ('-600', '-10 GMT (Hawaii Standard Time)'),
                                      ('-540', '-9 GMT (Alaska Standard Time)'),
                                      ('-480', '-8 GMT (Pacific Standard Time)'),
                                      ('-420', '-7 GMT (Mountain Standard Time)'),
                                      ('-360', '-6 GMT (Central Standard Time)'),
                                      ('-300', '-5 GMT (Eastern Standard Time)'),
                                      ('-240', '-4 GMT (Puerto Rico and US Virgin Islands Time)'),
                                      ('-180', '-3 GMT (Argentina Standard Time)'), ('-120', '-2 GMT'),
                                      ('-60', '-1 GMT (Central African Time)'), ('0', '0 GMT (Greenwich Mean Time)'),
                                      ('60', '+1 GMT (European Central Time)'),
                                      ('120', '+2 GMT (Eastern European Time)'),
                                      ('180', '+3 GMT (Eastern African Time)'), ('240', '+4 GMT (Near East Time)'),
                                      ('300', '+5 GMT (Pakistan Lahore Time)'),
                                      ('360', '+6 GMT (Bangladesh Standard Time)'),
                                      ('420', '+7 GMT (Vietnam Standard Time)'), ('480', '+8 GMT (China Taiwan Time)'),
                                      ('540', '+9 GMT (Japan Standard Time)'),
                                      ('600', '+10 GMT (Australia Eastern Time)'),
                                      ('660', '+11 GMT (Solomon Standard Time)')]}}
    ...
    def getLast(self):
        if self.last_list and 'ttl' in self.last_list:
            if self.last_list['ttl'] <> now.strftime('%Y-%m-%d') or 'ott' not in self.last_list or self.last_list[
                'ott'] <> self.OTT or 'server' not in self.last_list or self.last_list[
                'server'] <> self.ServerName or 'streamType' not in self.last_list or self.last_list[
    ...
    def getDefinedChannels(self):
        ...
        if os.path.isfile(file):
            ...
            try:
                ...
                xbmc.log('[SHURA.TV] Error loading file %s' % file)
            f.close()
    ...
    def getChannelsList(self):
        ...
        res = []
        defined = self.getDefinedChannels()
        ...
        for feed in dom.firstChild.childNodes:
            ...
            if feed.nodeName == 'feed':
                ...
                if name.encode('utf-8') in defined:
                    #xbmc.log(name)
                    res.append({
                        'id': id,
                        'name': name,
        ...
        self.last_list = {'channels': res, 'ttl': now.strftime('%Y-%m-%d'), 'ott': self.OTT, 'server': self.ServerName,
                          'streamType': self.StreamType}
    ...
    def getCurrentEPG(self, HOST, FEED):
        ...
        postparams = None
        try:
            req = urllib2.Request(url, postparams, {'User-agent': ua, 'Connection': 'Close',
        ...
        try:
            ...
            xbmc.log(
                '[SHURA.TV] Error in decoding json currentEPG' + str(e) + 'for' + str(resp) + 'len=' + str(len(resp)))
            resp2 = None
        else:
            resstream.close();
